...
export async function getFiltredPostsByQuery(query) {
  const results = await getClient(false).fetch(`*[_type == "house" && 
    commune match "*${query}*" ||
    province match "*${query}*" ||
    type match "*${query}*" ||
    category match "*${query}*" ||
    quartier match "*${query}*"
  ] | order(_createdAt desc){
      ${postFields}
    }[0...6]`);
  return getUniquePosts(results);
...
export async function getFiltredPostsByQueryCount(query, postRange) {
